  ...
  // mirror backend/search/ (tokenize.py, lexical.py); change them together.
  // ═══════════════════════════════════════════════════════════════════════
  const Search = (function () {
    ...
    // ── Passages, the shared unit of both indexes ───────────────────────
    ...
    const state = { lexical: 'loading', semantic: 'idle', semanticReason: '', semanticDetail: '' };
    const listeners = [];
    ...
    // The parsed query: content terms with their vocabulary expansions
    // (with an index) and the stemmed phrases. One entry is cached: the
    // query and the matcher parse the same text within a render.
    ...
    function parseQuery(query, smart) {
      ...
      const groups = L ? terms.map((term, i) => {
        const exact = L.termIndex.has(term) ? [L.termIndex.get(term)] : [];
        const fuzzy = (smart && !exact.length) ? nearTerms(term, i === terms.length - 1 && !phrases.length) : [];
        return { term, indices: exact.length ? exact : fuzzy, fuzzy: !exact.length };
      }) : [];
    ...
    function lexicalScores(parsed, opts) {
      ...
      parsed.groups.forEach(g => {
        // Expansions of one query word share its slot: a passage takes the best.
        const best = g.indices.length > 1 ? new Map() : null;
        g.indices.forEach(ti => {
          const w = idf(ti) * (g.fuzzy ? 0.7 : 1);
          if (w <= 0) return;
          const p = postingsOf(ti);
          for (let k = 0; k < p.n; k++) {
            if (speakers && !(p.spk[k] & speakers)) continue;
            const pid = p.pid[k];
            const dl = L.pLen[pid];
            const s = w * p.tf[k] * (L.k1 + 1) / (p.tf[k] + L.k1 * (1 - L.b + L.b * dl / L.avgdl));
            if (best) { if (s > (best.get(pid) || 0)) best.set(pid, s); }
            else addScore(acc, pid, s, g.term);
          }
        });
        if (best) best.forEach((s, pid) => addScore(acc, pid, s, g.term));
      });
      const needed = opts.exact ? parsed.terms.length : 1;
      const hits = new Map();
      acc.forEach((s, pid) => {
        if (s.matched.size < needed) return;
        if (parsed.phrases.length) {
          // Only a passage carrying every phrase word can hold the phrase.
    ...
    function addScore(acc, pid, s, term) {
      let slot = acc.get(pid);
      if (!slot) { slot = { score: 0, matched: new Set() }; acc.set(pid, slot); }
      slot.score += s; slot.matched.add(term);
    }

    // ── Meaning search: model + vectors ─────────────────────────────────
    let V = null;            // {n, d, q: Int8Array, s: Float32Array, model}
    let M = null;            // {session, output, maxTokens, queryPrefix, tokenizer}
    const vecCache = new Map();  // query text -> Float32Array
    let embedQueue = null;       // the newest text waiting for a vector
    let embedding = false;

    function wordPiece(vocabList, lowercase) {
      const ids = new Map(vocabList.map((t, i) => [t, i]));
      const CLS = ids.get('[CLS]'), SEP = ids.get('[SEP]'), UNK = ids.get('[UNK]');
      const isPunct = ch => {
      ...
      function basic(text) {
        ...
        for (const ch of text) {
          ...
          else if (isPunct(ch)) { if (cur) { out.push(cur); cur = ''; } out.push(ch); }
          else cur += ch;
        }
        if (cur) out.push(cur);
        return out;
      ...
      function pieces(tok) {
        if (tok.length > 100) return [UNK];
        const out = []; let start = 0;
        while (start < tok.length) {
          let end = tok.length, found = -1;
          while (start < end) {
            const sub = (start > 0 ? '##' : '') + tok.slice(start, end);
            if (ids.has(sub)) { found = ids.get(sub); break; }
            end--;
          }
          if (found === -1) return [UNK];
          out.push(found); start = end;
        }
        return out;
      }
      return {
        encode(text, maxTokens) {
          let out = [CLS];
          for (const tok of basic(text)) out.push(...pieces(tok));
          out = out.slice(0, maxTokens - 1); out.push(SEP);
          return out;
        },
      };
    }
    ...
    async function loadSemantic() {
      if (typeof WebAssembly === 'undefined') {
        state.semantic = 'unavailable'; state.semanticReason = 'nowasm';
        notify(); return;
      }
      ...
      try {
        ...
        const modelBytes = bytesOf(rawM.onnx);
        ort.env.wasm.numThreads = 1;
        ort.env.wasm.wasmBinary = wasmBytes.buffer;
        ort.env.wasm.wasmPaths = { mjs: URL.createObjectURL(new Blob([rawOrt.glue], { type: 'text/javascript' })) };
        const session = await ort.InferenceSession.create(modelBytes, { executionProviders: ['wasm'] });
        M = {
          session, output: session.outputNames[0], maxTokens: rawM.maxTokens, queryPrefix: rawM.queryPrefix || '',
          tokenizer: wordPiece(rawM.vocab.split('\n'), rawM.lowercase !== false),
        };
        state.semantic = 'ready';
      } catch (e) {
        state.semantic = 'unavailable'; state.semanticReason = 'failed';
        state.semanticDetail = e && e.message ? e.message : String(e);
      }
      notify();
    }

    async function embed(text) {
      const ids = M.tokenizer.encode(M.queryPrefix + text, M.maxTokens); const n = ids.length;
      const feeds = {
      ...
      const res = await M.session.run(feeds);
      const h = res[M.output]; const d = h.dims[2]; const v = new Float32Array(d);
      for (let t = 0; t < n; t++) for (let j = 0; j < d; j++) v[j] += h.data[t * d + j];
      let norm = 0; for (let j = 0; j < d; j++) { v[j] /= n; norm += v[j] * v[j]; }
      norm = Math.sqrt(norm) || 1; for (let j = 0; j < d; j++) v[j] /= norm;
      return v;
    }
    // The query vector if it is cached; otherwise queue the text. Only the
    // newest queued text is embedded (typing outruns the model), and the
    // page is notified when the vector it is waiting for lands.
    function queryVector(text) {
      const cached = vecCache.get(text);
      if (cached) return cached;
      embedQueue = text;
      if (!embedding) drainEmbeds();
      return null;
    }
    async function drainEmbeds() {
      embedding = true;
      while (embedQueue !== null) {
        const text = embedQueue; embedQueue = null;
        try { vecCache.set(text, await embed(text)); } catch (e) { /* the next query retries */ }
        if (embedQueue === null) notify();
      }
      embedding = false;
    }
    // Best passage per call by cosine.
    function semanticCallMax(vec, speakers) {
      const d = V.d; const q = V.q; const s = V.s;
      const bestScore = new Float32Array(CALLS.length).fill(-Infinity);
      const bestPid = new Int32Array(CALLS.length).fill(-1);
      for (let pid = 0; pid < V.n; pid++) {
        if (speakers && !(L.pSpk[pid] & speakers)) continue;
        let acc = 0; const o = pid * d;
        for (let j = 0; j < d; j++) acc += q[o + j] * vec[j];
        acc *= s[pid];
        const c = L.pCall[pid];
        if (acc > bestScore[c]) { bestScore[c] = acc; bestPid[c] = pid; }
      }
      return { bestScore, bestPid };
    }

    // ── Fusion (the rules tuned on the demo corpus) ─────────────────────
    const LEXICAL_WEIGHT = 0.7;      // keyword evidence leads
    const LEXICAL_CUTOFF = 0.3;      // calls under this share of the top keyword call are dropped
    const SEMANTIC_FLOOR = 0.45;     // a meaning-only call joins a keyword list only above this cosine
    const SEMANTIC_FALLBACK = 0.30;  // with no keyword hit at all: the top meaning-only calls above this
    const SEMANTIC_FALLBACK_MAX = 10;
    const EVIDENCE_PER_CALL = 3;

    /**
     * Run a query. Returns { calls: Map(call index -> {score, evidence}),
     * meaningOnly }. Evidence items are {pid, match, matched, needed}
     * (match: 'phrase' | 'words' | 'partial' | 'meaning'); without a
     * search index the page's own data is scanned with the matcher and the
     * items carry {text, start, speaker} instead of a pid. One entry is
     * memoized, so re-renders that do not change the query are free.
     */
    ...
    function query(text, opts) {
      ...
      const key = [text, smart, speakers, state.lexical, state.semantic, vecCache.has(text)].join(' ');
    ...
    function indexedQuery(text, smart, speakers) {
      ...
      if (!parsed.terms.length) return { calls: new Map(), meaningOnly: false };
      const lex = lexicalScores(parsed, { exact: !smart, speakers });

      // Keyword side: best passage per call, cut below a share of the top.
      const lexCalls = new Map();
      lex.forEach((s, pid) => {
        const c = L.pCall[pid]; const slot = lexCalls.get(c);
        if (!slot || s.score > slot.score) lexCalls.set(c, { score: s.score, pid });
      });
      let topLex = 0; lexCalls.forEach(v => { if (v.score > topLex) topLex = v.score; });
      if (smart) lexCalls.forEach((v, c) => { if (v.score < LEXICAL_CUTOFF * topLex) lexCalls.delete(c); });

      // Meaning side: Smart mode only, never for a numbers-only query.
      let sem = null;
      if (smart && state.semantic === 'ready' && !isDigits(parsed.terms.join(''))) {
        const vec = queryVector(text);
        if (vec) sem = semanticCallMax(vec, speakers);
      }
      const semScore = c => (sem && sem.bestPid[c] >= 0 ? sem.bestScore[c] : -Infinity);

      const combined = new Map();  // call -> {score, semPid, semScore}
      let meaningOnly = false;
      if (lexCalls.size) {
        const cands = new Set(lexCalls.keys());
        if (sem) for (let c = 0; c < CALLS.length; c++) if (semScore(c) >= SEMANTIC_FLOOR) cands.add(c);
        let lo = Infinity, hi = -Infinity;
        cands.forEach(c => { const s = Math.max(semScore(c), 0); lo = Math.min(lo, s); hi = Math.max(hi, s); });
        cands.forEach(c => {
          const lx = lexCalls.get(c);
          const ss = Math.max(semScore(c), 0);
          const ln = lx ? lx.score / topLex : 0;
          const sn = hi > lo ? (ss - lo) / (hi - lo) : 0;
          combined.set(c, { score: LEXICAL_WEIGHT * ln + (1 - LEXICAL_WEIGHT) * sn, semPid: sem ? sem.bestPid[c] : -1, semScore: ss });
        });
      } else if (sem) {
        meaningOnly = true;
        const above = [];
        for (let c = 0; c < CALLS.length; c++) if (semScore(c) >= SEMANTIC_FALLBACK) above.push(c);
        above.sort((a, b) => semScore(b) - semScore(a)).slice(0, SEMANTIC_FALLBACK_MAX)
          .forEach(c => combined.set(c, { score: semScore(c), semPid: sem.bestPid[c], semScore: semScore(c) }));
      }

      // Evidence per call: the best keyword passages (one per region, since
      // passages overlap), then the meaning passage.
      const byCall = new Map();
      lex.forEach((s, pid) => {
        const c = L.pCall[pid];
        if (!combined.has(c)) return;
        let arr = byCall.get(c); if (!arr) { arr = []; byCall.set(c, arr); }
        arr.push({ pid, score: s.score, matched: s.matched.size });
      });
      const needed = parsed.terms.length;
      const calls = new Map();
      combined.forEach((v, c) => {
        const kept = [];
        (byCall.get(c) || []).sort((a, b) => b.score - a.score).forEach(e => {
        ...
        const evidence = kept.map(e => ({
          pid: e.pid, matched: e.matched, needed,
          match: e.matched < needed ? 'partial' : (parsed.phrases.length ? 'phrase' : 'words'),
        }));
        if (v.semPid >= 0 && !evidence.some(e => e.pid === v.semPid) && (!evidence.length || v.semScore >= SEMANTIC_FLOOR)) {
          evidence.push({ pid: v.semPid, matched: 0, needed, match: 'meaning' });
        }
        calls.set(c, { score: v.score, evidence: evidence.slice(0, EVIDENCE_PER_CALL) });
      });
      return { calls, meaningOnly };
    }
    ...
    function scanQuery(text) {
      ...
      const calls = new Map();
      if (!mk) return { calls, meaningOnly: false };
      const needed = Math.max(mk.terms.length, 1);
      CALLS.forEach((call, idx) => {
        const evidence = [];
        for (const t of Data.turnsOf(call)) {
          if (evidence.length >= EVIDENCE_PER_CALL) break;
          if (mk.test(t.text)) evidence.push({ text: t.text, start: t.start, speaker: t.speaker, matched: needed, needed, match: 'words' });
        }
        const summary = summaryText(call);
        if (evidence.length < EVIDENCE_PER_CALL && mk.test(summary)) {
          evidence.push({ text: summary, start: null, speaker: 'SUMMARY', matched: needed, needed, match: 'words' });
        }
        if (evidence.length) calls.set(idx, { score: evidence.length, evidence });
      });
      return { calls, meaningOnly: false };
    }
    ...
    // A matcher for a query: hit(word) says whether a display word carries
    // one of the query's terms (memoized: display vocabularies are small),
    // test(text) whether a text does, mark(text) wraps the hits in <mark>.
    ...
    function matcher(text) {
      ...
      // Expansions (typos, prefixes) mark the vocabulary word they reached.
      parsed.groups.forEach(g => { if (g.fuzzy) g.indices.forEach(ti => termSet.add(L.vocab[ti])); });
      ...
      const hit = word => {
        ...
        let h = memo.get(w);
        if (h === undefined) {
          h = !STOPWORDS.has(w) && !FILLER.has(w) && indexTerms(w).some(t => termSet.has(t));
          memo.set(w, h);
        }
        return h;
      };
      if (!termSet.size) {
        return { terms: [], hit: () => false,
                 test: s => String(s || '').toLowerCase().indexOf(lower) !== -1,
                 mark: s => highlight(String(s == null ? '' : s), raw) };
      }
      return {
        terms: Array.from(termSet), hit,
        ...
        mark(s) {
          ...
          while ((m = WORD_RE.exec(str)) !== null) {
            ...
            out += hit(m[0]) ? '<mark>' + esc(m[0]) + '</mark>' : esc(m[0]);
    ...
    function snippet(text, mk, at) {
      if (mk && mk.terms.length) {
        WORD_RE.lastIndex = Math.max(0, at || 0);
        let m; at = -1;
        while ((m = WORD_RE.exec(text)) !== null) { if (mk.hit(m[0])) { at = m.index; break; } }
      } else at = -1;
    ...
    // the query's words (the first line for a meaning match); a summary
    // passage or a scanned summary has no time (start null).
    function evidence(item, mk) {
      ...
      lines.forEach(l => {
        const t = l.text.trim();
        const n = mk ? (t.match(WORD_RE) || []).reduce((a, w) => a + (mk.hit(w) ? 1 : 0), 0) : 0;
    ...
    function init() {
      injectScript('search/index.js').then(() => {
        ...
        loadLexical(raw);
        notify();
        // The meaning layer is optional (index.js says whether the package
        // has one). Its scripts load one at a time after keyword search is live.
        if (raw.semantic) setTimeout(loadSemantic, 50);
        else { state.semantic = 'none'; notify(); }
      }).catch(() => {
        state.lexical = 'missing';
        state.semantic = 'unavailable';
        notify();
      });
    }
    ...
    return {
      ...
      vectorReady(text) { return vecCache.has(String(text || '').trim()); },
      queryVector(text) { const v = vecCache.get(String(text || '').trim()); return v ? Array.from(v) : null; },
      encodeQuery(text) { return M ? M.tokenizer.encode(M.queryPrefix + text, M.maxTokens) : null; },
    };
  })();
