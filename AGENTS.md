...
**When editing `case_report_template.html`:** do a dry-run job end-to-end (even a small one) and spot-check the rendered PDF — its multi-section Paged.js layout uses absolute positioning, `.pagedjs_page` page-chrome backgrounds, and @page margins that can regress silently. Note that `.call-card` deliberately does NOT carry `page-break-inside: avoid`; that rule is scoped to `.call-card-head` only, so long cue tables flow cleanly across page breaks instead of orphaning the entire chapter header onto its own near-empty page.
